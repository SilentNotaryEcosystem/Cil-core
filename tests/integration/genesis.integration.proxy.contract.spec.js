...
let arrWitnesses;
let moneyIssueTx;
let strNewContractAddr;
const nAmountWitnessTwo = 1e5;
const nAmountWitnessThree = 3.3e7;
...
describe('Genesis net tests (it runs one by one!)', () => {
    ...
    it('should deploy new contract', async function() {
        ...
        const txCode = deployNewContract(wallet.privateKey, moneyIssueTx.hash(), 12);
        await witnessConciliumOne.rpcHandler({event: 'tx', content: txCode});
        ...
        const cTxReceipt = await witnessConciliumOne.rpcHandler({
            event: 'txReceipt',
            content: txCode.getHash()
        });

        strNewContractAddr = cTxReceipt.getContractAddress();
        assert.isOk(strNewContractAddr);
    ...
    it('should SET PROXY to new contract', async function() {
        ...
        const txCode = setProxy(strNewContractAddr, wallet.privateKey, moneyIssueTx.hash(), 13);
        await witnessConciliumOne.rpcHandler({event: 'tx', content: txCode});
    ...
    it('should join & start witness concilium', async function() {
        ...
        // use next (5th output)
        const txCode = joinConcilium(wallet.privateKey, moneyIssueTx.hash(), 5, nAmountWitnessTwo);
        await witnessConciliumTwo.rpcHandler({event: 'tx', content: txCode});
    ...
    it('should be 0 pending & 5 stable blocks', async () => {
    ...
    it('should LEAVE concilium 1 (send moneys back. withdraw from contract)', async function() {
        ...
        const txCode = leaveConcilium(wallet.privateKey, moneyIssueTx.hash(), 14);
        await witnessThree.rpcHandler({event: 'tx', content: txCode});
            ...
            const donePromise = new Promise((resolve, reject) => {
                sinon.stub(witnessThree, '_postAcceptBlock').callsFake((block) => {
                    if (block.txns.length === 2) {
                        ...
                        const receipt = result.patch.getReceipt(txCode.getHash());

                        const arrInternalTxns = receipt.getInternalTxns();
                        assert.equal(arrInternalTxns.length, 1);

                        const coins = receipt.getCoinsForTx(arrInternalTxns[0].toString('hex'));
                        assert.equal(coins.getAmount(), nAmountWitnessThree);
                        resolve();
                    } else {
                        reject();
                    }
    ...
    it('should check remaining BALANCE', async () => {
        ...
        const {patch} = witnessThree._pendingBlocks.getBlock(arrLastBlockHash[0]);
        const contract = patch.getContract(factory.Constants.CONCILIUM_DEFINITION_CONTRACT_ADDRESS);
...
/**
 * See _addPosConciliumMember. Changed height+ to +1
 *
 * @returns {string}
 */
...
function createGenesisBlock() {
    ...
    const contractDeployTx = factory.Transaction.createContract(createInitialContractCode(initialConcilium));
    contractDeployTx.signForContract(witnessOne.privateKey);
...
function deployNewContract(strClaimPrivateKey, utxo, idx) {
    const contractDeployTx = factory.Transaction.createContract(createFixedContractCode());

    contractDeployTx.addInput(utxo, idx);
    contractDeployTx.claim(0, strClaimPrivateKey);
...
function leaveConcilium(strClaimPrivateKey, utxo, idx) {
    console.log(`Using UTXo ${utxo} idx ${idx}`);
...
function setProxy(strNewAddress, strClaimPrivateKey, utxo, idx) {
    console.log(`Using UTXo ${utxo} idx ${idx}`);

    const contractCode = {
        method: 'setProxy',
        arrArguments: [strNewAddress]
    };
