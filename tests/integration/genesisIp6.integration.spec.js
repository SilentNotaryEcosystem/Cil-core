...
describe('Genesis net tests (localhost TCP. runs one by one!)', () => {
    ...
    it('should create initial witness and receive genesis', async function () {
        this.timeout(60000);
        ...
        await Promise.all([witnessConciliumOne.ensureLoaded(), witnessConciliumOne.bootstrap(), promiseBlock]);
    ...
    it('should create & start another witness concilium', async function () {
        ...
        await Promise.all([witnessConciliumTwo.ensureLoaded(), witnessConciliumTwo.bootstrap(), promiseBlock]);
    ...
    it('should create 3d node and load 4 blocks', async function () {
        ...
        await Promise.all([nodeThree.ensureLoaded(), nodeThree.bootstrap(), donePromise]);
    ...
    it('should create 4th node, that has Genesis, so it should load 3 blocks', async function () {
        ...
        await Promise.all([nodeFour.bootstrap(), donePromise]);

        assert.equal(nodeFour._pendingBlocks.getAllHashes().length, 1);
        assert.isAtLeast(nodeFour._mainDag.order, 4);
