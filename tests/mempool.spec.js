...
describe("Mempool tests", () => {
  ...
  it("should remove oldest txns", async function () {
  ...
  it("should remove outdated bad TXns", async function () {
    ...
    assert.isNotOk(
      mempool.isBadTx(hashTx1),
      "Successfully removed the outdated tx in BadTx"
    );
    assert.isNotOk(
      mempool.isBadTx(hashTx1),
      "Successfully removed the outdated tx in BadTx"
  ...
  it("should load local TXns from disk", async () => {
    ...
    assert.isOk(
      Array.isArray(mempool.getLocalTxnHashes()) &&
        mempool.getLocalTxnHashes().length === 1
    );
  });
  ...
  it("should getLocalTxnHashes", async () => {
    ...
    assert.deepEqual(mempool.getLocalTxnHashes(), [
      tx.getHash(),
      tx2.getHash(),
    ]);
  });
