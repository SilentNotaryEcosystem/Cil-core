...
const STR_CONTRACT_ADDR = 'CONTRACT ADDRESS HERE';

const objInvokeCode = {
    method: 'CONTRACT METHOD HERE',
    arrArguments: ['PARAMETER1', 'PARAMETER2']
};
...
async function main() {
    ...
    const {arrCoins} = cilUtils.gatherInputsForContractCall(arrUtxos, 50000);

    const tx = callContract(wallet, arrCoins);
    console.error(
 ...
 * @returns {*}
 */
function callContract(wallet, arrUtxos) {
    // todo: change here
    //   const tx = factory.Transaction.createContract(createContractCode(), wallet.address);
    const tx = factory.Transaction.invokeContract(STR_CONTRACT_ADDR, objInvokeCode, 50000, wallet.address);
