...
class Nft extends Base {
    constructor(nFeeDenominator = 10000) {
        ...
        // Owner -> [strReceiver, nRoyaltyFraction]
        this._defaultRoyaltyInfo = [];
    }
    ...
    createToken(objTokenData) {
        ...
        const strTokenId = this._sha256(`${callerAddress}:${strSymbol}`);
        ...
        this._data[strTokenId] = [
            ...
            callerAddress, // current owner
            null, // approved address
            [null, 0], // royaltyInfo
            block.hash
        ];
    ...
    balanceOf(strOwner) {
        ...
        this._balances[strOwner] || 0;
    }

    ownerOf(strTokenId) {
        return this.tokenData(strTokenId).strOwner;
    }
    ...
    approve(strSpender, strTokenId) {
        ...
        this._data[strTokenId][7] = strSpender;
    }

    getApproved(strTokenId) {
        return this.tokenData(strTokenId).strApproved;
    }
    ...
    totalSupply(strOwner = null) {
        if (!strOwner) return Object.keys(this._data).length;

        this._validateAddress(strOwner);

        return this._balances[strOwner];
    }

    tokenByIndex(nIndex) {
        this._validateParameterType(nIndex, 'string', 'nIndex');
        if (nIndex < 0 || nIndex > this.totalSupply()) throw new Error('nIndex out of range');

        const strTokenId = Object.keys(this._data)[nIndex];

        return this.tokenData(strTokenId).strTokenId;
    }

    tokenOfOwnerByIndex(strOwner, nIndex) {
        this._validateParameterType(nIndex, 'string', 'nIndex');
        if (nIndex < 0 || nIndex > this.totalSupply(strOwner)) throw new Error('nIndex out of range');
    }

    // ERC-2981: NFT Royalty Standard
    royaltyInfo(strTokenId, nSalePrice) {
        this._validateParameterType(strSymbolOrTokenId, 'string', 'strTokenId');
    ...
    _setTokenRoyalty(strTokenId, strReceiver, nFeeNumerator) {
        ...
        this._data[strTokenId][8] = [strReceiver, strFeeNumerator];
    }
    ...
    _resetTokenRoyalty(strTokenId) {
        ...
        this._data[strTokenId][8] = [null, 0];
    }
    ...
    _validateAddress(strAddress, strName) {
        this._validateParameterType(strAddress, 'string', strName);
        if (strAddress.length !== 40) throw new Error(`Bad address: ${strName}`);
    }
    ...
    _transfer(strFrom, strTo, strTokenId) {
        ...
        this._isApprovedOrOwner(strFrom, strTokenId);

        // // Clear approvals from the previous owner
        this._data[strTokenId][7] = null;

        this._balances[strFrom]--;
        this._balances[strTo] = (this._balances[strTo] || 0) + 1;

        this._data[strTokenId][6] = strTo;
    }
