...
module.exports = ({Constants, Crypto, Transaction}, {blockProto, spvBlockProto, blockHeaderProto}) =>
    class SPVBlock {
        ...
        _buildTxTree() {
            ...
            this._insideMerkleProofs =
                leaves.map(leaf => { return {hash: leaf.toString('hex'), proof: tree.getProof(leaf)}; });
            ...
            this._data.txnsHashes = this._data.txns
                .filter(tx => {
                    ...
                    return !(bloomFilter.test(txn.hash())
                             || txn.outputs &&
                             txn.outputs.some(out => bloomFilter.test(out.codeClaim.toString('hex'))));
                })
